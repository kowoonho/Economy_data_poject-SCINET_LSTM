...
class LevelSCINet(nn.Module):
    def __init__(self,in_planes, kernel_size, dropout, groups, hidden_size):
        ...
        self.interact = InteractorLevel(in_planes= in_planes, kernel = kernel_size, dropout = dropout, groups =groups , hidden_size = hidden_size)
...
class EncoderTree(nn.Module):
    def __init__(self, in_planes,  num_levels, kernel_size, dropout, groups, hidden_size):
        super().__init__()
...
class LSTM(nn.Module):
    def __init__(self, input_shape, output_shape):
        ...
        self.lstm_layer = nn.LSTM(self.input_shape, self.output_shape, num_layers=2)
        
    def forward(self, x):
        x = self.lstm_layer(x)
        x = x[0].to(device = 'cuda').clone()
        return x
...
class SCINet(nn.Module):
    def __init__(self, output_len, input_len, input_dim = 9, hid_size = 1, num_stacks = 1,
        ...
        self.lstm = LSTM(self.input_dim, self.input_dim)
